...
/***** Additional Interrupt functions to properly configure interrupt handling for stmpe/stmpe-ts drivers ****/

/*
 * Enable or disable interrupt on the rising edge of a event line
 */
void stm32_exti_enable_int_ts(unsigned int line, int enable)
{
	if (enable) {
		stm32_exti_clear_pending(line);

		stm32_exti_connect(line, 4);
...
EXPORT_SYMBOL(stm32_exti_enable_int_ts);

/*
 * Clear the pending state of a given event
 */
void stm32_exti_clear_pending_ts(unsigned int line)
{
	writel(1 << line, &STM32_EXTI->pr);
...
EXPORT_SYMBOL(stm32_exti_clear_pending_ts);
