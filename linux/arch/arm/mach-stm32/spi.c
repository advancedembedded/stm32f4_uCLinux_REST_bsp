...
#define SPI_FLASH_CS_PORT__STM32F4_REST_V1	3
#define SPI_FLASH_CS_PIN__STM32F4_REST_V1	11
	...
	else if (p == PLATFORM_STM32_STM_REST_V1) {
		bus = 1;
		port = SPI_FLASH_CS_PORT__STM32F4_REST_V1;
		pin = SPI_FLASH_CS_PIN__STM32F4_REST_V1;
	}
...
#		define FLASH_SIZE__DONGLE		(1024 * 1024 * 4)
		 ...
		 * SPI Flash partitioning:
		 */
#		define FLASH_JFFS2_OFFSET__DONGLE	(1024 * 1024 * 11)
#		define FLASH_SIZE__DONGLE		(1024 * 1024 * 5)
		static struct mtd_partition 
			spi_stm32_flash_partitions__dongle[] = {
			{
				.name = "spi_flash_part0",
		...
		/* take SPI flash out of reset and disable /wp */

		gpio_direction_output(STM32_GPIO_PORTPIN2NUM(1, 12), 1);
		gpio_direction_output(STM32_GPIO_PORTPIN2NUM(3, 12), 1);
