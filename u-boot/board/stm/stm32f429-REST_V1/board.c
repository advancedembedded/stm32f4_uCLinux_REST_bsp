...
static int fmc_fsmc_setup_gpio(void)
{
	int rv = 0;
	int i;
	static struct stm32f2_gpio_dsc	spi_nor_reset_gpio = {STM32F2_GPIO_PORT_B,STM32F2_GPIO_PIN_12};
	static struct stm32f2_gpio_dsc	spi_nor_wp_gpio = {STM32F2_GPIO_PORT_D,STM32F2_GPIO_PIN_12};

	
	rv = stm32f2_gpio_config(&spi_nor_reset_gpio, STM32F2_GPIO_ROLE_GPOUT);
	if (rv != 0)
		goto out;
	
	rv = stm32f2_gpio_config(&spi_nor_wp_gpio, STM32F2_GPIO_ROLE_GPOUT);
	if (rv != 0)
		goto out;

	stm32f2_gpout_set(&spi_nor_reset_gpio, 1);
	stm32f2_gpout_set(&spi_nor_wp_gpio, 1);
...
int checkboard(void)
{
	printf("Board: STM32F429-REST Rev %s\n", CONFIG_SYS_BOARD_REV_STR);
